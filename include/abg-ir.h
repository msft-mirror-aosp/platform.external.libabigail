...
///
/// As this is an optional uint64_t value, it allows the
/// representation of empty hash values.
typedef abg_compat::optional<uint64_t> hash_t;

/// The abstraction for a native offset.
///
/// This is to represent things like the DIE (in DWARF) offset of some
/// types.  It's useful to sort the types before type hashing.  As
/// this is an optional uint64_t value, it allows the representation
/// of empty offsets.
typedef abg_compat::optional<uint64_t> offset_t;
...
/// Helper typedef for an unordered set of type_base* which uses
/// pointer value to tell its members appart, because the members are
/// canonical types.
typedef unordered_set<type_base*,
		      canonical_type_hash> canonical_type_ptr_set_type;
...
struct shallow_type_hasher
{
  ...
  uint64_t
  operator()(const type_base_wptr& t) const
  {return operator()(t.lock());}
...
/// Convenience typedef for a set of @ref type_base_wptr
typedef unordered_set<type_base_wptr,
		      shallow_type_hasher,
		      shallow_type_eq> type_wptr_set_type;
...
public:
  ...
  const vector<type_base_sptr>* get_canonical_types(const string& name) const;
...
public:
  ...
  size_t
  get_artificial_value() const;

  void
  set_artificial(size_t v);
...
public:
  ...
  get_types_sorted() const;

  istring_type_base_wptrs_map_type*
  get_type_map(const std::type_info&);

  const istring_type_base_wptrs_map_type&
  get_type_map(const std::type_info&) const;
}; // end class type_maps;
...
public:
  ...
  offset_t
  get_native_offset() const;

  void
  set_native_offset(const offset_t);

  const corpus*
  get_corpus() const;

  void
  set_corpus(corpus*) const;
...
public:
  ...
  add_decl_to_scope(decl_base_sptr decl, scope_decl_sptr scpe);
...
public:
  ...
  get_sorted_member_types_copy() const;
  ...
  friend bool
  remove_member_decl(scope_decl_sptr scope, decl_base_sptr member);
...
public:
  ...
  global_scope(const environment& env);
...
public:
  ...
  dm_context_rel(scope_decl_sptr s,
		 bool is_laid_out,
  ...
  void
  set_anonymous_data_member(var_decl_sptr);
...
public:
  ...
  bool
  has_empty_parameters() const;

  unsigned
  get_nb_parameters() const;
...
public:
  ...
  friend void
  set_pattern(function_tdecl_sptr, function_decl_sptr p);
...
public:
  ...
  friend void
  set_pattern(class_tdecl_sptr, class_decl_sptr p);
...
public:
  ...
  member_functions
  get_member_functions_copy() const;

  const method_decl*
  find_member_function(const string& mangled_name) const;

  const method_decl*
  find_member_function(const char* mangled_name) const;
  ...
  friend decl_base_sptr
  insert_member_decl(class_or_union_sptr cou, decl_base_sptr member);

  friend bool
  remove_member_decl(class_or_union_sptr cou, decl_base_sptr member);

  friend decl_base_sptr
  add_member_decl(class_or_union_sptr cou, decl_base_sptr member_decl);

  friend void
  add_data_member(class_or_union_sptr cou,
		  var_decl_sptr v, access_specifier a,
		  bool is_laid_out, bool is_static,
		  size_t offset_in_bits);

  friend void
  add_member_function(class_or_union_sptr t,
		      method_decl_sptr f,
		      access_specifier a,
  ...
  friend void
  add_member_function_template(class_or_union_sptr,
			       member_function_template_sptr);

  friend void
  add_member_class_template(class_or_union_sptr,
...
public:
  ...
  base_specs
  get_base_specifiers_copy() const;
  ...
  decl_base_sptr
  insert_member_decl(class_decl_sptr klass, decl_base_sptr member);
...
public:
  ...
  mem_fn_context_rel(scope_decl_sptr s);
...
public:
  ...
  virtual bool visit_begin(method_type*);
  virtual bool visit_end(method_type*);
